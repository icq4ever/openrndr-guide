...
fun main() {
    ...
    두가지 방식으로 동작하는데, 첫번째는 흑백 사각형을 정적으로 배치 생성하는 것이고, 두번째는 동적으로 반투명한, 
    애니메이션 프레임이 있는 핑크색 사각형을 동적으로 배치 생성하는것입니다.
    
    `.rectangle()` 메소드는 두개의 인자를 갖습니다: `Rectangle` 오브젝트와 옵션 값인 회전값입니다. 
    `Rectangle`들은 위치와 가로 및 옵션으로 세로크기의 방법 혹은 `Rectangle.fromCenter()`를 사용하여 생성될 수 있습니다.
